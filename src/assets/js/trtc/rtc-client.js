...
export default class RtcClient {
  constructor(options) {
    ...
    TRTC.checkSystemRequirements().then(result => {
      if (!result) {
        alert("您的浏览器版本暂不支持视频面试，请使用最新版的谷歌浏览器");
      }
    });
  }
  ...
  async join() {
    ...
    try {
      ...
      try {
        ...
        this.vueObj.$message.error('当前麦克风无法使用，请检查您的电脑配置');
        await this.createLocalStream({ audio: false, video: true });
        //Toast.info('采集摄像头成功！');
      } catch (error) {
        this.vueObj.$message.error('当前摄像头无法使用，请检查您的电脑配置');
        return;
      }
    }
  ...
  handleEvents() {
    ...
    this.client_.on('stream-added', evt => {
      ...
      // 远端流默认已订阅所有音视频，此 处可指定只订阅音频或者音视频，不能仅订阅视频。
      // 如果不想观看该路远端流，可调用 this.client_.unsubscribe(remoteStream) 取消订阅
      //this.vueObj.checkInterviewPauseStatus();
      ...
      if (userinfo.usertype == '0' || userinfo.usertype == '2') {
        ...
        if (currentJobSeeker && currentJobSeeker.linkStatus != "4") {
          ...
          if (!this.remoteStreams_.find(x => x.getUserId() == remoteStream.getUserId())) {
    ...
    this.client_.on('stream-subscribed', evt => {
      const remoteStream = evt.stream;
      const id = remoteStream.getId();
      this.vueObj.remoteStreamsSubscribeStatusList[remoteStream.getUserId()] = true
      if (!this.remoteStreams_.find(x => x.getUserId() == remoteStream.getUserId())) {
        this.remoteStreams_.push(remoteStream);
        // setTimeout(() => {
    ...
    this.client_.on('stream-removed', evt => {
      const remoteStream = evt.stream;
      const id = remoteStream.getId();
      this.vueObj.remoteStreamsSubscribeStatusList[remoteStream.getUserId()] = false
      // 关闭远端流内部的音视频播放器
      remoteStream.stop();
